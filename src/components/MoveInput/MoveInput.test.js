...
test('User can enter a move', async () => {
	...
	expect(screen.getByRole("textbox").value).toBe('A1 B2');
});
...
test('Error message is removed after typing a character in the text field', async () => {
	...
	expect(screen.getByTestId('move-feedback')).not.toBeEmptyDOMElement();
...
test('Move changes based on what is typed', async () => {
	renderMoveInput();
	await updateInputValue("A1 B2");
	expect(screen.getByRole("textbox")).toHaveValue('A1 B2');
	await updateInputValue("A2 B");
	expect(screen.getByRole("textbox")).toHaveValue('A2 B');
	updateInputValue("A3 B3");
	expect(screen.getByRole("textbox")).toHaveValue('A3 B3');
});
...
test('Display an error message when no move is submitted', () => {
	renderMoveInput();
	fireEvent.click(screen.getByRole('button'));
	expect(screen.getByTestId('move-feedback')).toHaveTextContent('Please provide a move');
});
...
test('Display correct error message when only 1 position is provided', () => {
	...
	updateInputValue("C5")
	fireEvent.click(screen.getByRole('button'));
	expect(screen.getByTestId('move-feedback')).toHaveTextContent('Please provide a position to move to');
});
...
test('Display correct error message if too many positions are provided', () => {
	...
	fireEvent.click(screen.getByRole('button'));
	expect(screen.getByTestId('move-feedback')).toHaveTextContent('You may only move a piece to one position at a time');
});
...
test('Display correct error message when the first of 2 positions is invalid', () => {
	...
	fireEvent.click(screen.getByRole('button'));
	expect(screen.getByTestId('move-feedback')).toHaveTextContent('2g is not a valid position');
	...
	fireEvent.click(screen.getByRole('button'));
	expect(screen.getByTestId('move-feedback')).toHaveTextContent('B23 is not a valid position');
});
...
test('Display correct error message when the second of 2 positions is invalid', () => {
	...
	fireEvent.click(screen.getByRole('button'));
	expect(screen.getByTestId('move-feedback')).toHaveTextContent('z7 is not a valid position');
	...
	fireEvent.click(screen.getByRole('button'));
	expect(screen.getByTestId('move-feedback')).toHaveTextContent('H is not a valid position');
});
...
test('Display correct error message when both provided positions are not valid', () => {
	...
	fireEvent.click(screen.getByRole('button'));
	expect(screen.getByTestId('move-feedback')).toHaveTextContent('1A and 2B are not valid positions');
	updateInputValue("J9 z7");
	fireEvent.click(screen.getByRole('button'));
	expect(screen.getByTestId('move-feedback')).toHaveTextContent('J9 and z7 are not valid positions');
	updateInputValue("a12 B23");
	fireEvent.click(screen.getByRole('button'));
	expect(screen.getByTestId('move-feedback')).toHaveTextContent('a12 and B23 are not valid positions');
	updateInputValue("C H");
	fireEvent.click(screen.getByRole('button'));
	expect(screen.getByTestId('move-feedback')).toHaveTextContent('C and H are not valid positions');
});
...
test('Display an error when trying to move a piece that is not at the provided position', () => {
	...
	mockValidMove(null);
	fireEvent.click(screen.getByRole('button'));
	expect(screen.getByTestId('move-feedback')).toHaveTextContent('There isn\'t a piece at D2');
});
...
test('Display correct error message when attempting to move a black piece during white\'s turn', () => {
	...
	mockValidMove(blackPiece);
	fireEvent.click(screen.getByRole('button'));
	expect(screen.getByTestId('move-feedback')).toHaveTextContent('You may only move a white piece');
});
...
test('Displays piece specific error messages', () => {
	...
	fireEvent.click(screen.getByRole('button'));
	expect(screen.getByTestId('move-feedback')).toHaveTextContent('Error');
});
